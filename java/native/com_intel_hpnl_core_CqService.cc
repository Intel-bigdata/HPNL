...
static jfieldID _get_self_id(JNIEnv* env, jobject thisObj) {
  ...
  if (!init) {
    ...
    handleCqCallback =
        (*env).GetMethodID(cqServiceClassTmp, "handleCqCallback", "(JIII)V");
...
JNIEXPORT jint JNICALL Java_com_intel_hpnl_core_CqService_wait_1cq_1event(
    JNIEnv* env, jobject thisObj, jint index, jlong cqServicePtr) {
  ...
  if (!con) return -1;
  jlong jEq = *(jlong*)&eq;
  (*env).CallVoidMethod(thisObj, handleCqCallback, jEq, ret, buffer_id,
                        block_buffer_size);
  if (ret == RECV_EVENT) {
    std::cout << "wait event." << std::endl;
    if (con->activate_recv_chunk(ck, 0)) {
      // TODO: error handler
    }
 ...
 * Signature: (J)V
 */
JNIEXPORT jint JNICALL Java_com_intel_hpnl_core_CqService_init(JNIEnv* env,
                                                               jobject thisObj,
                                                               jlong eqService) {
  ExternalEqService* externalEqService = *(ExternalEqService**)&eqService;
  assert(externalEqService);
  ExternalCqService* externalCqService =
      new ExternalCqService(externalEqService, externalEqService->get_stack());
 ...
 * Signature: ()V
 */
JNIEXPORT void JNICALL Java_com_intel_hpnl_core_CqService_finalize(JNIEnv* env,
                                                                   jobject thisObj) {}
 ...
 * Signature: ()V
 */
JNIEXPORT void JNICALL Java_com_intel_hpnl_core_CqService_free(JNIEnv* env,
                                                               jobject thisObj,
                                                               jlong cqServicePtr) {
  ExternalCqService* service = *(ExternalCqService**)&cqServicePtr;
  if (service != NULL) {
