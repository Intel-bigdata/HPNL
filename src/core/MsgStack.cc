...
MsgStack::MsgStack(uint64_t flags_, int worker_num_, int buffer_num_, bool is_server_) : flags(flags_),
  worker_num(worker_num_), seq_num(0), buffer_num(buffer_num_), is_server(is_server_), fabric(nullptr), domain(nullptr), hints(nullptr), info(nullptr), hints_tmp(nullptr), info_tmp(nullptr),
...
int MsgStack::init() {
  ...
  if (fi_getinfo(FI_VERSION(1, 5), nullptr, nullptr, flags, hints, &info)) {
    perror("fi_getinfo");
    goto free_info;
  }
